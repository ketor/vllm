    ...
    def _prepare_decode_tensors(
        ...
        if not use_native and max_decode_len > 1:
            ...
            if min_decode_len == max_decode_len:
                ...
                # A (PP+MTP): clamp the copy width to the target buffer so the
                # kernel never writes past it. Under B's sizing
                # expanded_block_table.shape[1] == block_table.shape[1], so this
                # equals the full width and the tail zero-fill below is a no-op;
                # it stays as a defensive guard if the buffer width ever diverges
                # from the source width.
